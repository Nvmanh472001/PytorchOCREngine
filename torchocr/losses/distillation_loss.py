...
class CTCDKDLoss(nn.Module):
    ...
    def kl_loss(self, p1, p2):  # predict, label
        loss = torch.multiply(
            p2, torch.log((p2 + self.eps) / (p1 + self.eps) + self.eps))
        bs = loss.shape[0]
    ...
    def forward(self, logits_student, logits_teacher, targets, mask=None):
        ...
        pred_teacher_part2 = F.softmax(
            logits_teacher / self.temperature - 1000.0 * gt_mask_ex, dim=-1)
        pred_student_part2 = F.softmax(
            logits_student / self.temperature - 1000.0 * gt_mask_ex, dim=-1)
        # differents with dkd
